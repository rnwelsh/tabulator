...
class Cell extends CoreFeature{
  constructor(column, row){
    ...
    /** track if the cell has been added to the DOM yet */
    this.loaded = false; 
...
class Column extends CoreFeature{
  ...
  constructor(def, parent){
    ...
    /** column width */
    this.width = null; 
    /** column width pre-styled to improve render efficiency */
    this.widthStyled = ""; 
    /** column maximum width */
    this.maxWidth = null; 
    /** column maximum pre-styled to improve render efficiency */
    this.maxWidthStyled = ""; 
    this.maxInitialWidth = null;
    /** column minimum width */
    this.minWidth = null; 
    /** column minimum pre-styled to improve render efficiency */
    this.minWidthStyled = ""; 
    /** user has specified a width for this column */
...
class Row extends CoreFeature{
  constructor (data, parent, type = "row"){
    ...
    /** hold element height */
    this.height = 0; 
    /** hold element height pre-styled to improve render efficiency */
    this.heightStyled = ""; 
    /** user has manually set row height */
    this.manualHeight = false; 
    /** hold elements outer height */
    this.outerHeight = 0; 
    /** element has been rendered */
    this.initialized = false; 
    /** element has resized cells to fit */
    this.heightInitialized = false; 
    /** store position of element in row list */
    this.position = 0; 
...
class GroupRows extends Module$1{
  ...
  constructor(table){
    ...
    this.startOpen = [function(){return false;}]; 
...
class MoveColumns extends Module$1{
  ...
  constructor(table){
    ...
    /** floating column header element */
    this.hoverElement = false; 
    /** click check timeout holder */
    this.checkTimeout = false; 
    /** period to wait on mousedown to consider this a move and not a click */
    this.checkPeriod = 250; 
    /** currently moving column */
    this.moving = false; 
    /** destination column */
    this.toCol = false; 
    /** position of moving column relative to the destination column */
    this.toColAfter = false; 
    /** starting position within header element */
    this.startX = 0; 
    /** auto scroll on edge when within margin */
    this.autoScrollMargin = 40; 
    /** auto scroll distance in pixels */
...
class MoveRows extends Module$1{
  ...
  constructor(table){
    ...
    this.placeholderElement = this.createPlaceholderElement();
    /** floating row header element */
    this.hoverElement = false; 
    /** click check timeout holder */
    this.checkTimeout = false; 
    /** period to wait on mousedown to consider this a move and not a click */
    this.checkPeriod = 150; 
    /** currently moving row */
    this.moving = false; 
    /** destination row */
    this.toRow = false; 
    /** position of moving row relative to the destination row */
    this.toRowAfter = false; 
    /** row has handle instead of fully movable row */
    this.hasHandle = false; 
    /** starting Y position within header element */
    this.startY = 0; 
    /** starting X position within header element */
    this.startX = 0; 
...
class ReactiveData extends Module$1{
  ...
  constructor(table){
    ...
    this.blocked = false; 
    /** hold original data array functions to allow replacement after data is done with */
    this.origFuncs = {}; 
...
class SelectRow extends Module$1{
  ...
  constructor(table){
    ...
    /** flag selecting in progress */
    this.selecting = false; 
    /** last clicked row */
    this.lastClickedRow = false; 
    /** hold previously selected element for drag drop selection */
    this.selectPrev = []; 
    /** hold selected rows */
    this.selectedRows = []; 
    /** hold header select element */
...
class VirtualDomHorizontal extends Renderer{
  constructor(table){
    ...
    /** pixel margin to make column visible before it is shown on screen */
...
class ColumnManager extends CoreFeature {
  ...
  constructor (table){
    ...
    /** prevent redraws to allow multiple data manipulations before continuing */
    this.redrawBlock = false; 
    /** store latest redraw update only status */
...
class VirtualDomVertical extends Renderer{
  constructor(table){
    ...
    /** approximation of row heights for padding */
    this.vDomRowHeight = 20; 

    /** hold position for first rendered row in the virtual DOM */
    this.vDomTop = 0; 
    /** hold position for last rendered row in the virtual DOM */
    this.vDomBottom = 0; 

    /** last scroll position of the vDom top; */
    this.vDomScrollPosTop = 0; 
    /** last scroll position of the vDom bottom; */
    this.vDomScrollPosBottom = 0; 

    /** hold value of padding for top of virtual DOM */
    this.vDomTopPad = 0; 
    /** hold value of padding for bottom of virtual DOM */
    this.vDomBottomPad = 0; 

    /** the maximum number of dom elements that can be rendered in 1 go */
    this.vDomMaxRenderChain = 90; 

    /** window row buffer before removing elements, to smooth scrolling */
    this.vDomWindowBuffer = 0; 

    /** minimum number of rows to be generated in virtual dom (prevent buffering issues on tables with tall rows) */
    this.vDomWindowMinTotalRows = 20; 
    /** minimum number of rows to be generated in virtual dom margin */
    this.vDomWindowMinMarginRows = 5; 

    /** rows to normalize after appending to optimize render speed */
    this.vDomTopNewRows = []; 
    /** rows to normalize after appending to optimize render speed */
...
class RowManager extends CoreFeature{
  ...
  constructor(table){
    ...
    /** hold row data objects */
    this.rows = []; 
    /** hold calculation of active rows */
    this.activeRowsPipeline = []; 
    /** rows currently available to on display in the table */
    this.activeRows = []; 
    /** count of active rows */
    this.activeRowsCount = 0; 
    
    /** rows currently on display in the table */
    this.displayRows = []; 
    /** count of display rows */
    ...
    /** prevent redraws to allow multiple data manipulations before continuing */
    this.redrawBlock = false; 
    /** store latest redraw function calls for when redraw is needed */
    this.redrawBlockRestoreConfig = false; 
    /** store latest redraw function calls for when redraw is needed */
    this.redrawBlockRenderInPosition = false; 
    
    /** hold data pipeline tasks */
    this.dataPipeline = []; 
    /** hold data display pipeline tasks */
...
class DataLoader extends CoreFeature{
  constructor(table){
    ...
    /** prevent requests coming out of sequence if overridden by another load request */
    this.requestOrder = 0; 
    this.loading = false;
  }
...
class Localize extends Module$1{
  ...
  constructor(table){
    ...
    /** current language */
    this.lang = false; 
    /** update events to call when locale is changed */
    this.bindings = {}; 
    this.langList = {};
...
class Tabulator {
  ...
  constructor(element, options){
    ...
    /** holder Footer Manager */
    this.footerManager = null; 
    /** hold Alert Manager */
    this.alertManager = null; 
    /** holder horizontal virtual dom */
    this.vdomHoz  = null; 
    /** handle external event messaging */
    this.externalEvents = null; 
    /** handle internal event messaging */
    this.eventBus = null; 
    ...
    /** check if the table is in RTL mode */
    this.rtl = false; 
    /** hold original table element if it has been replaced */
    ...
    /** hold all modules bound to this table */
    this.modules = {}; 
    /** hold core modules bound to this table (for initialization purposes) */
    this.modulesCore = []; 
    /** hold regular modules bound to this table (for initialization purposes) */
    this.modulesRegular = []; 
